...
class UserFriendshipTest < ActiveSupport::TestCase
 ...
 test "that creating a friendship based on user id and friend id works" do
 	...
 	context "a new instance" do
 		setup do
 			@user_friendship = UserFriendship.new user: users(:eric), friend: users(:kevin)
 		end
 	...
 	context"#send_request_email" do 
 		setup do
 			@user_friendship = UserFriendship.create user: users(:eric), friend: users(:kevin)
 		end
 	...
 	context "#mutual_friendship" do
 		setup do 
 			UserFriendship.request users(:eric), users(:kevin)
 			@friendship1 = users(:eric).user_friendships.where(friend_id: users(:kevin).id).first
 			@friendship2 = users(:kevin).user_friendships.where(friend_id: users(:eric).id).first
 		end
 	...
 	context "#accept!" do 
 		setup do
 			@user_friendship = UserFriendship.request users(:eric), users(:kevin)
 		end
 	
 		should "set the sate to accepted" do 
 	...
 	context "on destroy" do 
  	...
  	context "#block!" do 
  		...
  		should "not allow new requests once blocked" do 
  			@user_friendship.block!
  			uf = UserFriendship.request users(:eric), users(:kevin)
  			assert !uf.save
  		end
  	end
end
