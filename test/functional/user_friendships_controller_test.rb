...
class UserFriendshipsControllerTest < ActionController::TestCase
  ...
  context "when logged in" do 
    setup do
      @friendship1 = create(:pending_user_friendship, user: users(:eric), friend: create(:user, first_name: 'Pending', last_name: 'Friend'))
      @friendship2 = create(:accepted_user_friendship, user: users(:eric), friend: create(:user, first_name: 'Active', last_name: 'Friend'))
      @friendship3 = create(:requested_user_friendship, user: users(:eric), friend: create(:user, first_name: 'Requested', last_name: 'Friend'))
      @friendship4 = user_friendships(:blocked_by_eric)
 ...
 context "#create" do 
  ...
  context "successfully" do 
    should "create two user friendship objects" do 
      assert_difference 'UserFriendship.count', 2 do 
        post :create, user_friendship: { friend_id: users(:kevin).profile_name}
      end
    end
  end
  ...
  should "create a friendship" do
    assert users(:eric).pending_friends.include?(users(:michelle))
  end
  ...
  should "set the flash success message" do 
    ...
    context "when logged in" do 
      setup do
        @friend = create(:user) 
        @user_friendship = create(:pending_user_friendship, user: users(:eric), friend: @friend)
        create(:pending_user_friendship, friend: users(:eric), user: @friend)
        sign_in users(:eric)
  ...
  context "when logged in" do 
    setup do
      @user_friendship = create(:pending_user_friendship, user: users(:eric))
      sign_in users(:eric) 
    ...
    context "when logged in" do 
      setup do 
        @friend = create(:user)
       @user_friendship = create(:accepted_user_friendship, friend: @friend, user: users(:eric))
       create(:accepted_user_friendship, friend: users(:eric), user: @friend)
